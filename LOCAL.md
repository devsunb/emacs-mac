...
- 마지막 GUI 프레임 삭제로 Prohibited가 되면 AppKit이 비동기로 앱을 hide 하는데, 그 사이 만든 새 프레임이 정책을 Regular로 되돌린 뒤에 도착하면 새 프레임까지 숨긴다. 그래서 정책 값이 아니라 Prohibited 전환이 세운 `activationPolicyHidePending`으로 사용자의 hide와 구분하고, 정책이 이미 Regular면 unhide로 되돌린다.
...
- `Assets.car`와 `CFBundleIconName`은 macOS 26 이상에서만 쓰이고, 그 아래에서는 `EmacsLG1-Default.icns`가 쓰인다.
