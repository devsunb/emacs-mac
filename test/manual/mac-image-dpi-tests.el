...
(ert-deftest mac-image-dpi-tests-scale-1-bitmap-is-smaller ()
  "A scale-1 frame rasterizes a high-DPI metadata image at 1x density."
  (skip-unless (and (eq window-system 'mac)
                    (display-images-p)
                    (image-type-available-p 'png)
                    (fboundp
               ...
               (mapcar
                (lambda (scale)
                  ...
                  (let ((cache-size (image-cache-size)))
                    (image-size image t)
                    (- (image-cache-size) cache-size)))
                '(1 2))))
          (should (< (car growth) (cadr growth))))
      (image-flush image frame)
      (image--set-test-frame-backing-scale-factor frame old-scale))))

(ert-deftest mac-image-dpi-tests-cache-hit ()
  "A second high-DPI metadata image lookup on a scale-1 frame hits the cache."
  (skip-unless (and (eq window-system 'mac)
                    (display-images-p)
                    (image-type-available-p 'png)
                    (fboundp
