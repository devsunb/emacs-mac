...
if there is one, otherwise any visible or iconified Mac frame.  If
there's no such frame, a new frame is created."
  (interactive "e")
  ...
  (let ((count 6))
    (while (and (> count 0)
      ...
      (sit-for 0.017)
      (setq count (1- count))))
  ;; The frontmost window may belong to a minimized frame or a child
  ;; frame; prefer a fully visible top-level one.
...
(defvar mac-minibuffer-deleted-frames nil
  "Mac frames whose deleted minibuffer should be aborted.")
...
Meant for `delete-frame-functions'."
  (when (and (eq (framep frame) 'mac)
    ...
    (push frame mac-minibuffer-deleted-frames)))
...
Meant for `after-delete-frame-functions'."
  (when (memq frame mac-minibuffer-deleted-frames)
    (setq mac-minibuffer-deleted-frames
	  (delq frame mac-minibuffer-deleted-frames))
