...
# mapfile needs bash 4 and "${empty[@]}" under set -u needs 4.4; the
# system /bin/bash is 3.2.
if ((BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 4))); then
  printf 'bash 4.4 or later is required (found %s).\n' "$BASH_VERSION" >&2
  exit 1
fi
...
macro_fingerprint() {
  { git ls-files -coz --exclude-standard -- 'lisp/*.el' |
      xargs -0 grep -slE \
        '^[[:blank:]]*\((defmacro|cl-defmacro|defsubst|cl-defsubst|cl-defstruct|define-inline) ' ||
      true; } |
...
install_up_to_date_p() {
  ...
  {
    IFS= read -r stamp_identity
    IFS= read -r stamp_fingerprint
  } 2>/dev/null <"$install_stamp"
...
# Remove everything git does not track except the protected paths and
# nested git repositories, which git clean reports as skipped.
git_clean_reset() {
  local protected='^\.claude(/|$)'
  # The output directory is protected only when it lies inside the tree.
  # git clean lists an untracked directory as a whole, so its ancestors
  # are skipped as well.
...
command -v gmake >/dev/null 2>&1 || {
  printf 'GNU Make (gmake) is required.\n' >&2
  exit 1
}
codesign_identity_available_p || {
...
if install_up_to_date_p; then
  ...
  # Only earlier runs' staging: must stay ahead of the mktemp below.
  rm -rf "$output_dir"/.Emacs.install.*
