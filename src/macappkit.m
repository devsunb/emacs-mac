		     ...
		     returnType:(NSPasteboardType)returnType
{
  if ([sendType length] == 0
	  ...
	  && [mac_service_selection_pasteboard ()
	       availableTypeFromArray:@[sendType]]))
    {
...
- (BOOL)readSelectionFromPasteboard:(NSPasteboard *)pboard
{
  ...
  if (result)
    {
      /* Storing the event needs the Lisp thread.  */
...
/* Rebuild the snapshots above from the keymaps and the selection
   ...
   where every AppKit reader runs.  */
...
update_action_and_services_handler_names (void)
{
  ...
  if (![action_names isEqualToSet:actions]
...
handle_action_invocation (NSInvocation *invocation)
{
  ...
  /* The deferred block must not come back to this thread (see
     ...
     collected above.  */
  mac_within_lisp_deferred_and_wake (^{
      handle_action_invocation_with_lisp (name, values, frame, modifiers);
    });
}
...
static pthread_mutex_t mac_deferred_lisp_queue_lock = PTHREAD_MUTEX_INITIALIZER;
  ...
  for (;;)
    {
      ...
      pthread_mutex_lock (&mac_deferred_lisp_queue_lock);
      queue = mac_deferred_lisp_queue;
      count = queue.count;
      if (count)
	mac_deferred_lisp_queue = [[NSMutableArray alloc] initWithCapacity:0];
      pthread_mutex_unlock (&mac_deferred_lisp_queue_lock);
      if (count == 0)
	break;
      do
	{
