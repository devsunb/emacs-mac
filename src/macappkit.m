...
- (void)discardCallbacksForNotificationRequests:
...
static BOOL applicationHiddenExplicitly, activationPolicyHidePending;

void
mac_set_activation_policy_prohibited (void)
{
  mac_within_gui (^{
      activationPolicyHidePending = ![NSApp isHidden];
...
{
  if (!activationPolicyHidePending)
...
- (void)applicationDidHide:(NSNotification *)notification
{
  if (activationPolicyHidePending)
    {
      activationPolicyHidePending = NO;
      if (NSApp.activationPolicy != NSApplicationActivationPolicyProhibited)
	[NSApp unhide:nil];
    }
}
  ...
  if ([event type] == NSEventTypeLeftMouseDown)
	...
	setupResizeTrackingSuspended = NO;
		  ...
		  == (NSEventModifierFlagControl | NSEventModifierFlagCommand)
	       && [self
		    respondsToSelector:@selector(performWindowDragWithEvent:)])
	{
	  self.movable = YES;
...
- (void)updateWindowStyle
{
  ...
  BOOL needsInternalToolBar = (!shouldBeTitled
...
{
  NSWindow *window = FRAME_MAC_WINDOW_OBJECT (f);
  bool activate_p = !(FRAME_TOOLTIP_P (f) || FRAME_NO_FOCUS_ON_MAP (f));
...
/* Requests awaiting the authorization verdict, oldest first so a
   batch drain preserves posting order.  */
static NSMutableArrayOf (UNNotificationRequest *)
  *mac_pending_notification_requests;

/* Requests handed to the notification center, by identifier.  */
...
mac_pending_notification_request_index (NSString *identifier)
{
  ...
  return [mac_pending_notification_requests
	   indexOfObjectPassingTest:^(UNNotificationRequest *request,
...
- (void)queueNotificationRequest:(UNNotificationRequest *)request
{
  ...
  if (mac_pending_notification_requests.count > MAC_PENDING_NOTIFICATION_LIMIT)
	...
	[mac_pending_notification_requests subarrayWithRange:range];
      ...
      [self discardCallbacksForNotificationRequests:dropped];
    }
...
- (void)submitNotificationRequest:(UNNotificationRequest *)request
{
  ...
  [center addNotificationRequest:request
	  ...
	  if (error)
	    [self discardCallbacksForNotificationRequests:@[request]];
	});
    }];
}
  ...
  (NSArrayOf (UNNotificationRequest *) *)requests
{
  ...
  NSMutableArrayOf (NSNumber *) *idNumbers =
    [NSMutableArray arrayWithCapacity:requests.count];

  for (UNNotificationRequest *request in requests)
    {
      NSNumber *idNumber = request.content.userInfo[@"id"];
  ...
  if (idNumbers.count != 0)
	...
	for (NSNumber *idNumber in idNumbers)
	  mac_notification_take_callback (idNumber.longLongValue, false);
      });
}
...
- (void)completeNotificationAuthorizationRequest:(BOOL)granted
{
  ...
  if (granted)
    ...
    [self discardCallbacksForNotificationRequests:requests];
}
...
/* Like mac_within_lisp_deferred, but also wake a Lisp thread sleeping
   in mac_select.  While the GUI thread is calling back into Lisp
   (mac_within_lisp) the queue is drained on the next inner Lisp round
   trip, after the GUI thread has left mac_gui_loop, so BLOCK must not
   call mac_within_gui: compute what needs the GUI thread first and
   pass it in.  */
