...
mac_update_window_begin (struct window *w)
{
  /* Resolve the remapped default face on the Lisp thread with the
     ...
     cannot leave it set.  w->contents can be nil in pseudo windows.  */
...
{
  /* cursor_default_face_id is resolved in mac_update_window_begin; no
     Lisp here, this also runs on the GUI thread.  */
  if ((!mac_reverse_video_cursor
       || s->face->ascii_face->id == s->w->cursor_default_face_id)
	  ...
	  && xgcv.foreground == s->face->foreground)
	{
      ...
      /* Reverse video: swap the face's colors, unless the swap would
