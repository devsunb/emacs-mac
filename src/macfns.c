  ...
  (Lisp_Object parms)
{
  ...
  FRAME_INTERNAL_TOOL_BAR_P (f)
    = undecorated || undecorated_round || !NILP (parent_frame);
...
mac_notification_prune (void)
{
  ...
  if (mac_notifications_callback_limit > 0)
    while (h->count >= mac_notifications_callback_limit)
      {
	...
	DOHASH (h, key, value)
	  {
	    intmax_t serial = XFIXNUM (XCAR (value));
...
have not yet been invoked.  When registering a new callback would leave
